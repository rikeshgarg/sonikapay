...
public class FragDatacard extends Fragment implements OnClickListener, WebServiceListener {
...
//        }
        ...
        if (response == 0) {
            ((ActivityRecharge)getActivity()).ShowConfirmDialog(svContext, strMessage, "datacard");
        }
    }

    public void ConfirmRecharge(){
        int amount = Integer.parseInt(edDthRechargeAmount.getText().toString().trim());
        boolean isWalletLoading = ((ActivityRecharge)getActivity()).checkRWalletAndAddWallet(amount, "datacard");
        if (isWalletLoading) {
