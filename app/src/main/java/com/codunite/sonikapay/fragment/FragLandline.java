...
public class FragLandline extends Fragment implements OnClickListener, WebServiceListener {
    private View aiView = null;
    private boolean mAlreadyLoaded = false;
    private TextView txtViewAllPlans, txtROffers;
    private RelativeLayout layOperator, layCircle;
    private Button btnDthrecharge;
    ...
    private int[] allViewWithClickId = {R.id.btn_dthrecharge, R.id.lay_operator,
            R.id.lay_circle, R.id.txt_viewallplans, R.id.txt_roffers};
...
//        }
        ...
        if (response == 0) {
            ((ActivityRecharge)getActivity()).ShowConfirmDialog(svContext, strMessage, "landline");
        }
    }

    public void ConfirmRecharge(){
        int amount = Integer.parseInt(edDthRechargeAmount.getText().toString().trim());
        boolean isWalletLoading = ((ActivityRecharge)getActivity()).checkRWalletAndAddWallet(amount, "landline");
        if (isWalletLoading) {
