...
import com.codunite.adapter.DMRCommisionAdapter;
...
public class ActivityDMRCommision extends AppCompatActivity implements View.OnClickListener, WebServiceListener {
    ...
    public void resumeApp() {
        ...
        callWebService(ApiInterface.GETMONEYTRANSFERCOMMISIONLIST, lstUploadData);
    ...
    private void StartApp() {
        ...
        errrorScreen = new NoInternetScreen(svContext, root, ActivityDMRCommision.this);
...
// FontUtils.setThemeColor(root, svContext, false);
    ...
    private List<DMRCommisionModel> lstItems = new ArrayList<>();

    @Override
    public void onWebServiceActionComplete(String result, String url) {
        if (url.contains(ApiInterface.GETMONEYTRANSFERCOMMISIONLIST)) {
            try {
                ...
                if (str_status.equalsIgnoreCase("0")) {
                    ...
                    for (int data_i = 0; data_i < ((JSONArray) data).length(); data_i++) {
                        ...
                        String trstartrange = data_obj.getString("start_range");
                        String strendrange = data_obj.getString("end_range");
                        String strflat = data_obj.getString("is_flat");
                        String strsurcharge = data_obj.getString("surcharge");
                        lstItems.add(new DMRCommisionModel(trstartrange, strendrange,strflat,strsurcharge));
                    }
                }
            ...
            DMRCommisionAdapter mAdapter = new DMRCommisionAdapter(this, lstItems, animation_type);
            wallethistoryrv.setNestedScrollingEnabled(false);
            wallethistoryrv.setAdapter(mAdapter);
            mAdapter.setOnItemClickListener(new DMRCommisionAdapter.OnItemClickListener() {
                @Override
